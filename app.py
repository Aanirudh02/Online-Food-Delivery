...
def get_images():
    ...
    cursor = connection.cursor(dictionary=True)  # Use dictionary cursor for easier access to columns
    cursor.execute("SELECT id, hotel_name, district, city, rating, timing, latitude, longitude, image_data FROM images")
...
def upload_image():
    if request.method == 'POST':
        ...
        if image:
            # Generate a unique image URL using the timestamp
            image_url = f"images/{int(time.time())}_{image.filename}"  # Save the filename with timestamp
            ...
            # Optionally, save the image to the filesystem if needed
...
def view_images():
    ...
    cursor.execute("SELECT id, hotel_name, city, district, rating, timing, latitude, longitude, image_data FROM images")
    images = cursor.fetchall()

    # Format images to include base64 data and Google Maps link
    formatted_images = []
    for img in images:
        image_data_base64 = base64.b64encode(img[8]).decode('utf-8')  # Convert binary to Base64
        
        # Create Google Maps link using latitude and longitude
        maps_link = f"https://www.google.com/maps/search/?api=1&query={img[6]},{img[7]}"
        
        formatted_images.append({
            'id': img[0],  # Include the id in the formatted data
            'hotel_name': img[1],
            'city': img[2],
            'district': img[3],
            'rating': img[4],
            ...
            'latitude': img[6],
            'longitude': img[7],
            'img_src': image_data_base64,  # Use the Base64 string
            'maps_link': maps_link  # Include the Google Maps link
        })

    cursor.close()
    connection.close()

    return render_template('view_images.html', images=formatted_images)
...
def edit_image(image_id):
    ...
    if image:
        ...
        # Update image data
        cursor.execute(
            "UPDATE images SET hotel_name=%s, district=%s, city=%s, rating=%s, timing=%s, latitude=%s, longitude=%s "
            "WHERE id=%s",
...
def view_imog():
    ...
    return render_template('view_imog.html', images=formatted_images)
